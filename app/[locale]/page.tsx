...
export default async function HomePage({
  ...
  return (
    <div className="mx-auto max-w-[1440px] animate-rise-slow">
      ...
      >
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-5">
          {FEATURES.map((feature, index) => (
            ...
            >
              <span className="pa-numeric text-[14px] font-bold text-pa-ghost">
                {String(index + 1).padStart(2, "0")}
              </span>
              <h3 className="font-display text-[17px] leading-snug text-pa-text">
      ...
      >
        <span className="text-[15px] text-pa-faint">
        ...
        <nav className="flex flex-wrap gap-6">
          {(["privacy", "terms", "changelog", "support"] as const).map((key) => (
            <span
              key={key}
              className="font-display text-[11px] uppercase tracking-[.1em] text-pa-ghost"
            >
              {t(`footer.${key}`)}
            </span>
          ))}
